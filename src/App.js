...
export default function NewGame(props) {
    ...
    function select(e) {// 落子
        if (gameOver === true) {
            if (gameOver && alert(`游戏已结束，获胜者是${e === 1 ? "X" : "O"},是否重置游戏？`)) {
        ...
        if (realXIndex >= 0 && realYIndex >= 0 && realXIndex <= 20 && realYIndex <= 20 && history[realXIndex][realYIndex] === 0) {
            ...
            ctx.closePath();
            setIsWhite(!isWhite);
            dispatch({ type: SELECT, realXIndex, realYIndex, user: isWhite ? 1 : 2 })
        };
    }
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                <div>
                    {
                        gameOver ? `游戏已结束，获胜者是${isWhite ? "白棋" : "黑棋"}` : "棋局仍在进行中"
                    }
                </div>
                <div>当前执棋者&nbsp;{isWhite ? "黑棋" : "白棋"}</div>
